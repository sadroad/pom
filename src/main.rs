...
fn notify_user(stream: &rodio::OutputStream, alert_file: &'static [u8], body: &str) {
    let _ = Notification::new().summary("Pomodoro").body(body).show();
...
fn main() {
    ...
    loop {
        ...
        pb.finish_and_clear();
        notify_user(&stream_handle, alert_file, "Work Done");
        ...
        pb.finish_and_clear();
        notify_user(&stream_handle, alert_file, "Break Over");
    }
}
